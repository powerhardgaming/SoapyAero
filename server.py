...
CARD_DEFINITIONS = [
    {
        ...
        "color":    "#00ccff",
        "emoji":    "🌈",
        "desc":     "Customize your username color or gradient. Everyone sees it.",
        "sell_price": 10,
    },
    {
        "id":       "rih",
        ...
        "color":    "#ff66cc",
        "emoji":    "😏",
        "desc":     "Plays the rizz sound effect for everyone online. 10s cooldown.",
        "sell_price": 40,
        "cooldown": 10,
    },
    {
        ...
        "color":    "#aaaaaa",
        "emoji":    "🔩",
        "desc":     "Plays metal pipe SFX and shows a random metal pipe image for 10 seconds. 15s cooldown.",
        "sell_price": 120,
        "cooldown": 15,
    },
    {
        ...
        "color":    "#ffe066",
        "emoji":    "🍼",
        "desc":     "Draws a baby oil bottle on everyone's screen for 30 seconds. 60s cooldown.",
        "sell_price": 400,
        "cooldown": 60,
    },
    {
        ...
        "color":    "#ffcc44",
        "emoji":    "⭕",
        "desc":     "Floats a translucent Cheerio on everyone's screen for 1 minute. 3s cooldown.",
        "sell_price": 600,
        "cooldown": 3,
    },
    {
        ...
        "color":    "#ff2222",
        "emoji":    "👁",
        "desc":     "24 hours of full server admin. Kick, delete groups, change server params. One-time use.",
        "sell_price": 9999,
        "cooldown": 0,
    },
]
...
async def god_delete_account(request):
    ...
    await broadcast_all({"type":"system_msg","room":"public","content":f"⚡ Account '{target}' was deleted by SoapyGod {actor}"})
    return web.json_response({"ok":True})

async def god_set_params(request):
    """POST /api/god/params  — change global server params during god session."""
...
async def god_revert_check(_app):
    """Background task: auto-revert SoapyGod after 24 hours."""
    global MAX_MESSAGES, MAX_CHARS
    while True:
        ...
        with cards_lock:
            ...
            expired = [(u, s) for u, s in god_sessions.items()
        ...
        for username, session in expired:
            ...
            # Restore deleted users from snapshot
            ...
            with users_lock:
                for u, ph in snap_users.items():
                    if u not in users_db:
                        users_db[u] = ph
                save_json(USERS_FILE, users_db)
            # Restore deleted groups from snapshot
            ...
            with groups_lock:
                for gid, g in snap_groups.items():
                    if gid not in groups_db:
                        groups_db[gid] = g
                save_json(GROUPS_FILE, groups_db)
            # Mark as reverted so we don't re-run
            with cards_lock:
            ...
            # Broadcast restored groups
            with groups_lock:
                grps = list(groups_db.values())
            await broadcast_all({"type":"groups_list","groups":[public_group(g) for g in grps]})
            print(f"[SoapyGod] Session for {username} expired and reverted.", flush=True)
